...
export default function Store() {
   ...
   * ============================================================
   *
   * Un compte Store n'est PAS automatiquement un abonné.
   *
   * Le véritable droit d'accès vient de :
   *
   * subscriptions
   *   kind = "bib_subscriber"
   *   status = "active" | "trialing"
   *
   * Le paiement Stripe et le webhook déterminent donc
   * l'accès réel aux fonctionnalités abonnées.
   */
  ...
  useSEO({
    title:
      "BIB Store — Découvrez des boutiques vérifiées",
  ...
  const goToAccount = () => {
    ...
    navigate(
      "/store/login?next=%2Fstore%2Fsubscribe",
    );
  };
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-5 sm:px-6 md:flex-row md:items-center md:justify-between lg:px-8">
        <div>
          <p className="font-medium">
            {isStoreAccount
              ? "Votre compte BIB est prêt."
              : "Vous découvrez BIB pour la première fois ?"}
          </p>

          <p className="mt-1 text-sm text-muted-foreground">
            {isStoreAccount
              ? "Activez maintenant BIB Abonné pour accéder aux fonctionnalités réservées aux abonnés."
              : "Créez votre compte puis activez BIB Abonné à 4,99 €/mois pour accéder aux fonctionnalités réservées aux abonnés."}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {isStoreAccount
              ? "Activer BIB Abonné · 4,99 €/mois"
              : "Devenir BIB Abonné · 4,99 €/mois"}
          </button>
