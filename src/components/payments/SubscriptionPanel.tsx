...
export function SubscriptionPanel() {
  ...
  const { plan: currentPlan, tier, billingCycle, insuranceAddonEnabled } = useCurrentPlan();
  const { data: subs = [] } = useUserSubscriptions();
  const portal = useOpenBillingPortal();

  const [annual, setAnnual] = useState(billingCycle === "annual");
  const [checkoutPriceId, setCheckoutPriceId] = useState<string | null>(null);

  const sortedPlans = [...plans].sort((a, b) => a.sort_order - b.sort_order);
  const insuranceSub = subs.find(
    (s) => s.price_id.startsWith("insurance_") && ["active", "trialing"].includes(s.status),
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-bib-gold/30 bg-bib-gold/5 p-4 sm:p-5">
        <div className="flex items-start gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-1">
              ...
              {insuranceAddonEnabled && (
                <Badge className="bg-info/15 text-info hover:bg-info/15 text-[10px] gap-1">
                  <ShieldCheck className="h-3 w-3" /> Assurance active
                </Badge>
              )}
            </div>
            {currentPlan && (
              <p className="text-xs text-muted-foreground">
                Commission {currentPlan.commission_percent}% · jusqu'à{" "}
                {currentPlan.max_boutiques} boutique{currentPlan.max_boutiques > 1 ? "s" : ""}{" "}
                · {currentPlan.max_products ? `${currentPlan.max_products} produits` : "produits illimités"}
              </p>
            )}
          </div>
          <Button
          ...
          >
            ...
            <span className="ml-1.5">Gérer ma facturation</span>
          </Button>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-3 rounded-full border border-border bg-card px-4 py-2 w-fit mx-auto">
        ...
        <Switch checked={annual} onCheckedChange={setAnnual} aria-label="Cycle annuel" />
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isCurrent ? "border-bib-gold ring-2 ring-bib-gold/30" : "border-border",
                !isCurrent && featured && "border-bib-gold/40",
              )}
            >
              {isCurrent && (
              ...
              <ul className="space-y-1.5 text-xs text-foreground/80 flex-1 mb-3">
                {plan.features.slice(0, 4).map((f) => (
                  <li key={f} className="flex items-start gap-1.5">
                    <Check className="h-3 w-3 text-success shrink-0 mt-0.5" /> {f}
                  </li>
                ))}
              </ul>
              <Button
      ...
      {currentPlan && (
        <div className="rounded-2xl border border-info/30 bg-info/5 p-4 sm:p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                <p className="font-display font-semibold text-foreground">Add-on Assurance</p>
                {insuranceAddonEnabled ? (
                  <Badge className="bg-success/15 text-success hover:bg-success/15 text-[10px]">
                    Active
                  </Badge>
                ) : (
                  <Badge variant="outline" className="text-[10px]">Non active</Badge>
                )}
              </div>
              <p className="text-xs text-muted-foreground mb-3">
                Couverture jusqu'à <strong>{currentPlan.insurance_per_dispute_cap_eur}€/litige</strong>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {insuranceAddonEnabled ? (
                  ...
                  <Button size="sm" variant="coral" onClick={subscribeInsurance}>
                    <Sparkles className="h-3.5 w-3.5 mr-1.5" />
                    Activer l'assurance
                  </Button>
                )}
              </div>
              {insuranceSub?.cancel_at_period_end && (
                <p className="text-[11px] text-warning mt-2">
                  Annulation programmée le{" "}
                  {insuranceSub.current_period_end
                    ? new Date(insuranceSub.current_period_end).toLocaleDateString("fr-FR")
                    : "—"}
                </p>
              )}
            </div>
