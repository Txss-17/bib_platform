...
export function CheckoutForm({ boutiqueId, boutiqueName, primaryColor, onBack }: CheckoutFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Insert one row per item; the DB trigger generates the real LKS26-XXXXXX number.
      ...
      // Show the first order number (or a list if multiple)
      const numbers = (created ?? []).map((o: any) => o.order_number).filter(Boolean);
      setOrderNumber(numbers[0] ?? "");
