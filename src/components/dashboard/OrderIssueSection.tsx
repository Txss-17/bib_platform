...
interface OrderIssueSectionProps {
  orderId: string;
}

export function OrderIssueSection({
  orderId,
}: OrderIssueSectionProps) {
  const { data: issues = [], isLoading } = useOrderIssues(orderId);
  ...
  if (issues.length === 0) {
    return (
      <p className="text-xs text-muted-foreground italic">
        Aucun signalement pour cette commande.
      </p>
    );
  }
      ...
      <CardContent className="p-3 space-y-2">
        ...
        {issue.status === "pending" && (
          <div className="flex items-center justify-between text-xs">
            ...
            >
              {isOverdue
                ? "⚠️ Délai dépassé — escalade automatique"
                : `⏳ ${hoursLeft}h restantes pour répondre`}
            </span>
          </div>
        )}
              ...
              >
                ...
                <div>
                  <span className="font-medium">
                    {ISSUE_ACTION_LABELS[response.action]}
                  </span>
        ...
        {showResponse && (
          <div className="space-y-2 pt-2 border-t border-border/30">
            <Select
              value={action}
              onValueChange={(value) =>
                setAction(value as IssueAction)
              }
            >
              ...
              <SelectContent>
                {(Object.keys(
                  ISSUE_ACTION_LABELS,
                ) as IssueAction[]).map((actionKey) => (
                  <SelectItem
                    key={actionKey}
                    value={actionKey}
                  >
                    {ISSUE_ACTION_LABELS[actionKey]}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
