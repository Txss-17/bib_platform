import { useState } from "react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Textarea } from "@/components/ui/textarea";
import { 
  Package, ShoppingBag, Eye, CheckCircle2, Camera, 
  Loader2, AlertTriangle, ArrowRight, Upload 
} from "lucide-react";
import { 
  useSampleValidation, useOrderSample, useReceiveSample, useValidateSample,
  getSampleStatusLabel, getSampleStatusColor, type SampleStatus
} from "@/hooks/useSampleValidation";
import { toast } from "sonner";

interface SampleValidationPanelProps {
  productId: string;
  productName: string;
  compact?: boolean;
}

const steps = [
  { key: "ordered" as const, label: "Commander", icon: ShoppingBag, description: "Commander un échantillon" },
  { key: "received" as const, label: "Tester", icon: Eye, description: "Tester le produit" },
  { key: "validated" as const, label: "Valider", icon: CheckCircle2, description: "Valider l'échantillon" },
];

function getStepIndex(status: SampleStatus): number {
  switch (status) {
    case "none": return -1;
    case "ordered": return 0;
    case "received": return 1;
    case "validated": return 2;
  }
}

export function SampleValidationPanel({ productId, productName, compact = false }: SampleValidationPanelProps) {import { useState } from "react";
import {
  AlertTriangle,
  ArrowRight,
  CheckCircle2,
  Loader2,
  Package,
  ShoppingBag,
} from "lucide-react";
import { toast } from "sonner";

import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Textarea } from "@/components/ui/textarea";

import {
  useOrderSample,
  useReceiveSample,
  useSampleValidation,
  useValidateSample,
} from "@/hooks/useSampleValidation";

interface SimpleValidationSectionProps {
  productId: string;
  productName: string;
  compact?: boolean;
}

type SampleValidationStatus =
  | "none"
  | "ordered"
  | "received"
  | "validated";

interface ValidationRecord {
  status?: SampleValidationStatus;
  comment?: string | null;
}

interface ValidationStep {
  key: SampleValidationStatus;
  label: string;
  icon: typeof Package;
}

const steps: ValidationStep[] = [
  {
    key: "ordered",
    label: "Commandé",
    icon: ShoppingBag,
  },
  {
    key: "received",
    label: "Reçu",
    icon: Package,
  },
  {
    key: "validated",
    label: "Validé",
    icon: CheckCircle2,
  },
];

function getStepIndex(status: SampleValidationStatus): number {
  switch (status) {
    case "ordered":
      return 0;
    case "received":
      return 1;
    case "validated":
      return 2;
    default:
      return -1;
  }
}

function getSampleStatusLabel(status: SampleValidationStatus): string {
  switch (status) {
    case "ordered":
      return "Échantillon commandé";
    case "received":
      return "Échantillon reçu";
    case "validated":
      return "Produit validé";
    default:
      return "Échantillon non commandé";
  }
}

function getSampleStatusColor(status: SampleValidationStatus): string {
  switch (status) {
    case "ordered":
      return "bg-primary/10 text-primary border-primary/30";
    case "received":
      return "bg-warning/10 text-warning border-warning/30";
    case "validated":
      return "bg-success/15 text-success border-success/30";
    default:
      return "bg-muted text-muted-foreground border-border";
  }
}

export function SimpleValidationSection({
  productId,
  productName,
  compact = false,
}: SimpleValidationSectionProps) {
  const { data: validation, isLoading } =
    useSampleValidation(productId);

  const orderSample = useOrderSample();
  const receiveSample = useReceiveSample();
  const validateSample = useValidateSample();

  const [comment, setComment] = useState("");
  const [showValidateForm, setShowValidateForm] = useState(false);

  if (isLoading) {
    return null;
  }

  const validationRecord = validation as ValidationRecord | null | undefined;

  const status: SampleValidationStatus =
    validationRecord?.status ?? "none";

  const currentStep = getStepIndex(status);

  const handleOrder = async () => {
    try {
      await orderSample.mutateAsync(productId);

      toast.success("Échantillon commandé", {
        description: `Votre échantillon de "${productName}" a été commandé. Vous serez notifié à sa réception.`,
        duration: 5000,
      });
    } catch {
      toast.error("Erreur lors de la commande");
    }
  };

  const handleReceive = async () => {
    try {
      await receiveSample.mutateAsync(productId);

      toast.success("Échantillon reçu", {
        description: `"${productName}" est maintenant marqué comme reçu. Testez-le avant de valider son activation.`,
        duration: 5000,
      });
    } catch {
      toast.error("Erreur lors de l'enregistrement de la réception");
    }
  };

  const handleValidate = async () => {
    const trimmedComment = comment.trim();

    if (!trimmedComment) {
      toast.error(
        "Veuillez ajouter un commentaire sur votre retour d'expérience",
      );
      return;
    }

    try {
      await validateSample.mutateAsync({
        productId,
        comment: trimmedComment,
      });

      toast.success("Produit validé et activé", {
        description: `"${productName}" est maintenant visible et disponible à la vente dans votre boutique.`,
        duration: 6000,
      });

      setComment("");
      setShowValidateForm(false);
    } catch {
      toast.error("Erreur lors de la validation");
    }
  };

  if (status === "validated" && compact) {
    return (
      <Badge className="text-[10px] bg-success/15 text-success border-success/30">
        <CheckCircle2 className="w-3 h-3 mr-1" />
        Validé
      </Badge>
    );
  }

  if (compact) {
    return (
      <Badge
        className={`text-[10px] border ${getSampleStatusColor(status)}`}
      >
        {getSampleStatusLabel(status)}
      </Badge>
    );
  }

  return (
    <Card className="border-border/50">
      <CardContent className="p-4 space-y-4">
        {/* Header */}
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            <Package className="w-4 h-4 text-primary shrink-0" />

            <h4 className="text-sm font-semibold">
              Validation de l’échantillon
            </h4>
          </div>

          <Badge
            className={`text-[10px] border shrink-0 ${getSampleStatusColor(
              status,
            )}`}
          >
            {getSampleStatusLabel(status)}
          </Badge>
        </div>

        {/* Information */}
        {status !== "validated" && (
          <div className="flex items-start gap-2 p-3 rounded-lg bg-warning/10 border border-warning/30">
            <AlertTriangle className="w-4 h-4 text-warning mt-0.5 shrink-0" />

            <p className="text-xs text-warning">
              Vous devez tester cet échantillon avant d'activer le
              produit à la vente dans votre boutique.
            </p>
          </div>
        )}

        {/* Workflow */}
        <div className="flex items-center gap-1">
          {steps.map((step, index) => {
            const done = currentStep >= index;
            const active =
              currentStep === index - 1 ||
              (currentStep === -1 && index === 0);

            const StepIcon = step.icon;

            return (
              <div
                key={step.key}
                className="flex items-center gap-1 flex-1 min-w-0"
              >
                <div
                  className={`flex items-center gap-1.5 p-2 rounded-lg flex-1 text-xs transition-colors ${
                    done
                      ? "bg-success/15 text-success"
                      : active
                        ? "bg-primary/10 text-primary"
                        : "bg-muted text-muted-foreground"
                  }`}
                >
                  <StepIcon className="w-3.5 h-3.5 shrink-0" />

                  <span className="font-medium hidden sm:inline truncate">
                    {step.label}
                  </span>
                </div>

                {index < steps.length - 1 && (
                  <ArrowRight
                    className={`w-3 h-3 shrink-0 ${
                      done
                        ? "text-success"
                        : "text-muted-foreground/30"
                    }`}
                  />
                )}
              </div>
            );
          })}
        </div>

        {/* Order sample */}
        {status === "none" && (
          <Button
            onClick={handleOrder}
            disabled={orderSample.isPending}
            className="w-full gap-2"
            size="sm"
          >
            {orderSample.isPending ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
              <ShoppingBag className="w-4 h-4" />
            )}

            Commander un échantillon
          </Button>
        )}

        {/* Receive sample */}
        {status === "ordered" && (
          <Button
            onClick={handleReceive}
            disabled={receiveSample.isPending}
            className="w-full gap-2"
            variant="outline"
            size="sm"
          >
            {receiveSample.isPending ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
              <Package className="w-4 h-4" />
            )}

            J'ai reçu mon échantillon
          </Button>
        )}

        {/* Open validation form */}
        {status === "received" && !showValidateForm && (
          <Button
            onClick={() => setShowValidateForm(true)}
            className="w-full gap-2"
            size="sm"
          >
            <CheckCircle2 className="w-4 h-4" />

            Valider mon échantillon
          </Button>
        )}

        {/* Validation form */}
        {status === "received" && showValidateForm && (
          <div className="space-y-3 p-3 rounded-lg bg-muted/50">
            <div>
              <p className="text-xs font-medium">
                Votre retour d'expérience
              </p>

              <p className="text-[11px] text-muted-foreground mt-1">
                Décrivez brièvement la qualité et votre expérience
                avec le produit.
              </p>
            </div>

            <Textarea
              value={comment}
              onChange={(event) => setComment(event.target.value)}
              placeholder="Qualité, texture, rendu, conformité..."
              className="text-sm min-h-[80px]"
            />

            <div className="flex gap-2">
              <Button
                variant="outline"
                size="sm"
                onClick={() => {
                  setShowValidateForm(false);
                  setComment("");
                }}
                className="flex-1"
              >
                Annuler
              </Button>

              <Button
                size="sm"
                onClick={handleValidate}
                disabled={
                  validateSample.isPending || !comment.trim()
                }
                className="flex-1 gap-2"
              >
                {validateSample.isPending ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
                  <CheckCircle2 className="w-4 h-4" />
                )}

                Valider
              </Button>
            </div>
          </div>
        )}

        {/* Validated state */}
        {status === "validated" && (
          <div className="flex items-start gap-2 p-3 rounded-lg bg-success/10 border border-success/30">
            <CheckCircle2 className="w-4 h-4 text-success shrink-0 mt-0.5" />

            <div className="min-w-0">
              <p className="text-xs font-medium text-success">
                Produit validé et actif
              </p>

              {validationRecord?.comment && (
                <p className="text-[10px] text-success mt-0.5 break-words">
                  "{validationRecord.comment}"
                </p>
              )}
            </div>
          </div>
        )}
      </CardContent>
    </Card>
  );
}
