...
/**
 * Espace client en panneau latéral.
 *
 * Permet au client de rester sur la page courante tout en accédant à :
 * - ses favoris
 * - ses cartes cadeaux
 * - ses commandes
 * - ses paramètres
 *
 * Gère également l'authentification et l'activation du profil client.
 */
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <NotLoggedInState onClose={() => onOpenChange(false)} />
          ) : profileLoading ? (
            <Centered>
              <Loader2 className="h-5 w-5 animate-spin text-white/60" />
            </Centered>
...
function NotLoggedInState({ onClose }: { onClose: () => void }) {
  return (
    <div className="p-6 space-y-4">
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-center">
        ...
        <p className="text-sm text-white/70">
          Connectez-vous pour accéder à vos favoris, vos cartes cadeaux, vos
          commandes et vos paramètres.
        </p>
      </div>
...
function ActivateProfileState({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (claimedCount > 0) {
        toast.success(
          `${claimedCount} commande${
            claimedCount > 1 ? "s" : ""
          } rattachée${claimedCount > 1 ? "s" : ""} 🎉`,
        );
      } else {
  ...
  return (
    <form onSubmit={submit} className="p-6 space-y-4">
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 flex items-center gap-3">
        ...
        <p className="text-sm text-white/70">
          Activez votre espace pour suivre vos commandes et cumuler des cartes
          cadeaux.
        </p>
      </div>
      ...
      <div className="space-y-1.5">
        <Label htmlFor="cp-name" className="text-white/70">
          Votre nom (optionnel)
        </Label>
      ...
      <label className="flex items-start gap-2 cursor-pointer text-sm text-white/70">
        ...
        <span>
          Je souhaite recevoir les bons plans des boutiques BIB.
        </span>
      </label>
  ...
  return (
    <Tabs defaultValue={initialTab} className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-6">
        ...
        <TabsContent value="giftcards" className="mt-2">
          <GiftCardsTab />
        </TabsContent>
...
function FavoritesTab() {
  ...
  if (favorites.length === 0 || favoriteItems.length === 0) {
    return (
      <EmptyDark
        icon={Heart}
        text="Aucun favori. Touchez le ❤️ d'un produit pour le retrouver ici."
      />
    );
  }
...
function GiftCardsTab() {
  ...
  if (cards.length === 0) {
    return (
      <EmptyDark
        icon={Gift}
        text="Recyclez vos cartons pour gagner des cartes cadeaux ! 1 point = 0,10 €"
      />
    );
  }
  ...
  return (
    <div className="space-y-3">
      {cards.map((c: any) => (
        <div
          key={c.id}
          className="overflow-hidden rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/20 to-white/[0.02]"
        >
          <div className="p-4 flex items-center gap-3">
            {c.boutiques?.logo_url ? (
              <img
                src={c.boutiques.logo_url}
                alt=""
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                {STATUS_LABELS[o.logistics_status] ??
                  o.logistics_status}
              </Badge>
            </div>
          ...
          <div className="text-right">
            ...
            {o.boutiques?.slug && (
              <Link
                to={`/boutique/${o.boutiques.slug}/order-tracking`}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <Stat
          label="Points"
          value={customer?.total_recycling_points ?? 0}
          accent
        />
      </div>
