...
async function handleCheckoutCompleted(session: any) {
  ...
  if (kind !== "storefront") return; // subscriptions handled by customer.subscription.*

  const orderIds = (session.metadata?.orderIds || "").split(",").filter(Boolean);
  if (!orderIds.length) {
    console.warn("storefront checkout completed without orderIds");
    return;
  }
  await getSupabase()
    .from("orders")
